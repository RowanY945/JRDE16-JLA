...
WITH parsed_data AS (
    SELECT 
        ...
        -- Map employment type to standardized enum values
        CASE
            WHEN job_employment_type IS NULL OR TRIM(job_employment_type) = '' THEN NULL
            WHEN lower(regexp_replace(job_employment_type, '[^a-z]', '')) = 'fulltime' THEN 'FULL_TIME'
            WHEN lower(regexp_replace(job_employment_type, '[^a-z]', '')) = 'parttime' THEN 'PART_TIME'
            WHEN lower(regexp_replace(job_employment_type, '[^a-z]', '')) = 'contract' THEN 'CONTRACT'
            WHEN lower(regexp_replace(job_employment_type, '[^a-z]', '')) = 'internship' THEN 'INTERNSHIP'
            WHEN lower(regexp_replace(job_employment_type, '[^a-z]', '')) = 'temporary' THEN 'TEMPORARY'
            WHEN lower(regexp_replace(job_employment_type, '[^a-z]', '')) = 'volunteer' THEN 'VOLUNTEER'
            ELSE UPPER(job_employment_type)
    ...
    FROM {{ source('bronze_layer', 'indeed_jobs') }}
